...
#include <unistd.h>
  ...
  /**
   * @brief   frame_gray_create()으로 생성된 Frame 객체와 내부 버퍼를 모두 해제합니다.
   ...
   * @param   frame   [in] Frame 포인터
   * @return  const unsigned char* (NULL 허용)
   */
  ...
  /**
   * @brief   Frame의 픽셀 데이터를 파일 디스크립터에서 읽습니다. (EOF 시 자동으로 처음부터 다시
   * 읽기)
   * @param   fd      [in] 파일 디스크립터 (읽기 가능)
